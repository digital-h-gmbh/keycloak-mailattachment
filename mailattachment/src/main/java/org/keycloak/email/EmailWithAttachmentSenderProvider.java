    ...
    private void addResources(Multipart multipart, Map<String, String> config) {
        if (include != null) {
            try {
                ...
                if (parent) {
//                   throw new Error('multiple files not jet implemented');
                } else {
...
//                log.warn("Failed to attach files", me);
    ...
    private String getResourcePath(String resPath) throws IOException {
        ...
        String themeRoot = "theme/" + theme.getName() + "/" + theme.getType().toString().toLowerCase() + "/";
        String resourceRoot = themeRoot + "resources/";
        String relativePath = resourceRoot + resPath;
